...
w.searchPanes.panes&&(null===d||1===d.draw))for(this.c.cascadePanes||this._reloadSelect(w),d=0,w=w.searchPanes.panes;d<w.length;d++)a=w[d],a.id===this.s.index&&(a.searchTerm&&0<a.searchTerm.length&&(this.dom.searchBox.val(a.searchTerm),this.dom.searchBox.trigger("input")),this.s.dtPane.order(a.order).draw(),a.collapsed?this.collapse():this.show());return!0};g.prototype._detailsPane=function(){var a=this.s.dt;this.s.rowData.arrayTotals=[];this.s.rowData.binsTotal={};var b=this.s.dt.settings()[0];a=
a.rows().indexes();if(!this.s.dt.page.info().serverSide)for(var d=0;d<a.length;d++)this._populatePaneArray(a[d],this.s.rowData.arrayTotals,b,this.s.rowData.binsTotal)};g.prototype._displayPane=function(){var a=this.dom.container,b=this.s.colOpts,d=parseInt(this.c.layout.split("-")[1],10);this.dom.topRow.empty();this.dom.dtP.empty();this.dom.topRow.addClass(this.classes.topRow);3<d&&this.dom.container.addClass(this.classes.smallGap);this.dom.topRow.addClass(this.classes.subRowsContainer);this.dom.upper.appendTo(this.dom.topRow);
