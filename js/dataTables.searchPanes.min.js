...
i=Object.keys(g.binsOriginal).length;m=this._uniqueRatio(i,c.rows()[0].length);if(!1===this.s.displayed&&((void 0===e.show&&null===e.threshold?m>this.c.threshold:m>e.threshold)||!0!==e.show&&1>=i)){this.dom.container.addClass(this.classes.hidden);this.s.displayed=!1;return}this.c.viewTotal&&0===g.arrayTotals.length?this._detailsPane():g.binsTotal=g.bins;this.dom.container.addClass(this.classes.show);this.s.displayed=!0}}else this.s.displayed=!0;this._displayPane();this.dom.dtP.on("stateLoadParams.dt",
function(a,b,d){$.isEmptyObject(c.state.loaded())&&$.each(d,function(a){delete d[a]})});i=$.fn.dataTable.ext.errMode;$.fn.dataTable.ext.errMode="none";m=n.Scroller;this.s.dtPane=$(this.dom.dtP).DataTable($.extend(!0,{columnDefs:[{className:"dtsp-nameColumn",data:"display",render:function(a,b,c){if(b==="sort")return c.sort;if(b==="type")return c.type;var f;d.s.filteringActive&&d.c.viewTotal?f=h.replace(/{total}/,c.total):f=k.replace(/{total}/,c.total);for(f=f.replace(/{shown}/,c.shown);f.indexOf("{total}")!==
-1;)f=f.replace(/{total}/,c.total);for(;f.indexOf("{shown}")!==-1;)f=f.replace(/{shown}/,c.shown);b="";f='<span class="'+d.classes.pill+'">'+f+"</span>";if(d.c.hideCount||e.hideCount)f="";return b=d.c.dataLength?a.length>d.c.dataLength?'<span class="'+d.classes.name+'">'+a.substr(0,d.c.dataLength)+"...</span>"+f:'<span class="'+d.classes.name+'">'+a+"</span>"+f:'<span class="'+d.classes.name+'">'+a+"</span>"+f},targets:0,type:void 0!==c.settings()[0].aoColumns[this.s.index]?c.settings()[0].aoColumns[this.s.index]._sManualType:
null},{className:"dtsp-countColumn "+this.classes.badgePill,data:"total",targets:1,visible:!1}],deferRender:!0,info:!1,paging:m?!0:!1,scrollY:"200px",scroller:m?!0:!1,select:!0,stateSave:c.settings()[0].oFeatures.bStateSave?!0:!1},this.c.dtOpts,void 0!==e?e.dtOpts:{},null!==this.customPaneSettings&&void 0!==this.customPaneSettings.dtOpts?this.customPaneSettings.dtOpts:{}));this.s.listenerActive||this._actListeners();$(this.dom.dtP).addClass(this.classes.table);$(this.dom.searchBox).attr("placeholder",
void 0!==e.header?e.header:this.colExists?c.settings()[0].aoColumns[this.s.index].sTitle:this.customPaneSettings.header||"Custom Pane");$.fn.dataTable.select.init(this.s.dtPane);$.fn.dataTable.ext.errMode=i;if(this.colExists){var f=(f=f.search())?f.substr(1,f.length-2).split("|"):[],p=0;g.arrayFilter.forEach(function(a){""===a.filter&&p++});i=0;for(f=g.arrayFilter.length;i<f;i++)if(this.s.dt.page.info().serverSide&&(!this.c.cascadePanes||this.c.cascadePanes&&0!==g.bins[g.arrayFilter[i].filter]||this.c.cascadePanes&&
null!==b)){m=this._addRow(g.arrayFilter[i].display,g.arrayFilter[i].filter,g.bins[g.arrayFilter[i].filter],this.c.viewTotal?String(g.binsTotal[g.arrayFilter[i].filter]):g.bins[g.arrayFilter[i].filter],g.arrayFilter[i].sort,g.arrayFilter[i].type);void 0!==e.preSelect&&-1!==e.preSelect.indexOf(g.arrayFilter[i].filter)&&m.select();for(var j=0,o=this.s.serverSelect;j<o.length;j++)o[j].filter===g.arrayFilter[i].filter&&(this.s.serverSelecting=!0,m.select(),this.s.serverSelecting=!1)}else!this.s.dt.page.info().serverSide&&
g.arrayFilter[i]&&(void 0!==g.bins[g.arrayFilter[i].filter]||!this.c.cascadePanes)?(m=this._addRow(g.arrayFilter[i].display,g.arrayFilter[i].filter,g.bins[g.arrayFilter[i].filter],g.binsTotal[g.arrayFilter[i].filter],g.arrayFilter[i].sort,g.arrayFilter[i].type),void 0!==e.preSelect&&-1!==e.preSelect.indexOf(g.arrayFilter[i].filter)&&m.select()):this.s.dt.page.info().serverSide||this._addRow(this.c.emptyMessage,p,p,this.c.emptyMessage,this.c.emptyMessage,this.c.emptyMessage)}(void 0!==e.options||null!==
this.customPaneSettings&&void 0!==this.customPaneSettings.options)&&this._getComparisonRows();n.select.init(this.s.dtPane);this.s.dtPane.draw();this.s.dt.on("stateSaveParams.dtsp",function(a,b,c){if($.isEmptyObject(c))d.s.dtPane.state.clear();else{var a=[],e,f,k,i;void 0!==d.s.dtPane&&(a=d.s.dtPane.rows({selected:!0}).data().map(function(a){return a.filter.toString()}).toArray(),e=$(d.dom.searchBox).val(),f=d.s.dtPane.order(),k=g.binsOriginal,i=g.arrayOriginal);void 0===c.searchPanes&&(c.searchPanes=
{});void 0===c.searchPanes.panes&&(c.searchPanes.panes=[]);c.searchPanes.panes.push({arrayFilter:i,bins:k,id:d.s.index,order:f,searchTerm:e,selected:a})}});if(l&&l.searchPanes&&l.searchPanes.panes){this.c.cascadePanes||this._reloadSelect(l);f=0;for(l=l.searchPanes.panes;f<l.length;f++)i=l[f],i.id===this.s.index&&($(this.dom.searchBox).val(i.searchTerm),this.s.dt.order(i.order))}$(this.dom.nameButton).on("click.dtsp",function(){var a=d.s.dtPane.order()[0][1];d.s.dtPane.order([0,"asc"===a?"desc":"asc"]).draw()});
$(this.dom.countButton).on("click.dtsp",function(){var a=d.s.dtPane.order()[0][1];d.s.dtPane.order([1,"asc"===a?"desc":"asc"]).draw()});$(this.dom.clear).on("click.dtsp",function(){d.dom.container.find("."+d.classes.search).each(function(){$(this).val("");$(this).trigger("input")});d.clearPane()});$(this.dom.searchButton).on("click.dtsp",function(){$(d.dom.searchBox).focus()});$(this.dom.searchBox).on("input.dtsp",function(){d.s.dtPane.search($(d.dom.searchBox).val()).draw();d.s.dt.state.save()});
this.s.dt.state.save();return!0};h.prototype._actListeners=function(){var a=this,b;this.s.dtPane.on("deselect.dtsp",function(){b=setTimeout(function(){a.s.dt.page.info().serverSide&&!a.s.updating?a.s.serverSelecting||(a.s.serverSelect=a.s.dtPane.rows({selected:!0}).data().toArray(),a.s.deselect=!0,a.s.dt.draw(!1)):(a.s.deselect=!0,0===a.s.dtPane.rows({selected:!0}).data().toArray().length&&$(a.dom.clear).addClass(a.classes.dull),a._makeSelection(),a.s.deselect=!1,a.s.dt.state.save())},50)});this.s.dtPane.on("select.dtsp",
...
d.searchPanes&&(d.searchPanes={});d.searchPanes.selectionList=b.s.selectionList});0<this.s.selectionList.length&&this.c.cascadePanes&&this._cascadeRegen(this.s.selectionList);a.columns(0<this.c.columns.length?this.c.columns:void 0).eq(0).each(function(a){if(void 0!==b.s.panes[a]&&void 0!==b.s.panes[a].s.dtPane&&void 0!==b.s.panes[a].s.colOpts.preSelect)for(var c=b.s.panes[a].s.dtPane.rows().data().toArray().length,d=0;d<c;d++)-1!==b.s.panes[a].s.colOpts.preSelect.indexOf(b.s.panes[a].s.dtPane.cell(d,
