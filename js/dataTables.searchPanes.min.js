...
if(void 0===f.settings()[0]._searchPanes){this._getState();if(this.s.dt.page.info().serverSide)f.on("preXhr.dt",function(a,b,d){void 0===d.searchPanes&&(d.searchPanes={});a=0;for(b=c.s.selectionList;a<b.length;a++){var f=b[a],g=c.s.dt.column(f.index).dataSrc();void 0===d.searchPanes[g]&&(d.searchPanes[g]={});for(var i=0;i<f.rows.length;i++)d.searchPanes[g][i]=f.rows[i].filter}});f.on("xhr",function(a,b,d){d&&(d.searchPanes&&d.searchPanes.options)&&(c.s.serverData=d,c.s.serverData.tableLength=d.recordsTotal,
...
return a};k.prototype.getNode=function(){return this.dom.container};k.prototype.rebuild=function(a,b){void 0===a&&(a=!1);void 0===b&&(b=!1);i(this.dom.emptyMessage).remove();var d=[];!1===a&&i(this.dom.panes).empty();for(var c=0,f=this.s.panes;c<f.length;c++){var h=f[c];!1!==a&&h.s.index!==a||(h.clearData(),d.push(h.rebuildPane(void 0!==this.s.selectionList[this.s.selectionList.length-1]?h.s.index===this.s.selectionList[this.s.selectionList.length-1].index:!1,this.s.dt.page.info().serverSide?this.s.serverData:
void 0,null,b)),i(this.dom.panes).append(h.dom.container))}this.s.dt.page.info().serverSide||this.s.dt.draw();this.c.cascadePanes||this.c.viewTotal?this.redrawPanes(!0):this._updateSelection();this._updateFilterCount();this._attachPaneContainer();this.s.dt.draw();return 1===d.length?d[0]:d};k.prototype.redrawPanes=function(a){void 0===a&&(a=!1);var b=this.s.dt;if(!this.s.updating&&!this.s.dt.page.info().serverSide){var d=!0,c=this.s.filterPane;if(b.rows({search:"applied"}).data().toArray().length===
...
void 0)e.searchPanes={};for(var c=a=0,d=b.s.panes;c<d.length;c++){var f=d[c],h=b.s.dt.column(f.s.index).dataSrc();e.searchPanes[h]===void 0&&(e.searchPanes[h]={});if(f.s.dtPane!==void 0)for(var f=f.s.dtPane.rows({selected:true}).data().toArray(),g=0;g<f.length;g++){e.searchPanes[h][g]=f[g].filter;a++}}b.c.viewTotal&&b._prepViewTotal();if(a>0){e.start=0;b.s.dt.page(0)}});else a.on("preXhr.dt",function(){for(var a=0,c=b.s.panes;a<c.length;a++)c[a].clearData()});this.s.dt.on("xhr",function(){var a=false;
if(!b.s.dt.page.info().serverSide)b.s.dt.one("preDraw",function(){if(!a){var c=b.s.dt.page();a=true;i(b.dom.panes).empty();for(var e=0,d=b.s.panes;e<d.length;e++){var f=d[e];f.clearData();f.rebuildPane(b.s.selectionList[b.s.selectionList.length-1]!==void 0?f.s.index===b.s.selectionList[b.s.selectionList.length-1].index:false,void 0,void 0,true);i(b.dom.panes).append(f.dom.container)}b.s.dt.page.info().serverSide||b.s.dt.draw();b.c.cascadePanes||b.c.viewTotal?b.redrawPanes(b.c.cascadePanes):b._updateSelection();
b._checkMessage();b.s.dt.one("draw",function(){b.s.dt.page(c).draw(false)})}})});d=0;for(c=this.s.panes;d<c.length;d++)if(e=c[d],void 0!==e&&void 0!==e.s.dtPane&&(void 0!==e.s.colOpts.preSelect&&0<e.s.colOpts.preSelect.length||null!==e.customPaneSettings&&void 0!==e.customPaneSettings.preSelect&&0<e.customPaneSettings.preSelect.length)){f=e.s.dtPane.rows().data().toArray().length;for(h=0;h<f;h++)(-1!==e.s.colOpts.preSelect.indexOf(e.s.dtPane.cell(h,0).data())||null!==e.customPaneSettings&&void 0!==
e.customPaneSettings.preSelect&&-1!==e.customPaneSettings.preSelect.indexOf(e.s.dtPane.cell(h,0).data()))&&e.s.dtPane.row(h).select();e.updateTable()}if(void 0!==this.s.selectionList&&0<this.s.selectionList.length){d=this.s.selectionList[this.s.selectionList.length-1].index;c=0;for(f=this.s.panes;c<f.length;c++)e=f[c],e.s.lastSelect=e.s.index===d}0<this.s.selectionList.length&&this.c.cascadePanes&&this._cascadeRegen(this.s.selectionList);this._updateFilterCount();a.on("destroy.dtsps",function(){for(var c=
